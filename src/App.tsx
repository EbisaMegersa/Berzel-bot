...
export default function App() {
  ...
  const handleWinStar = async () => {
    if (isWatching || !auth.currentUser) return;
    
    setIsWatching(true);

    const rewardStar = async () => {
      if (!auth.currentUser) return;
      const userDocPath = `users/${auth.currentUser.uid}`;
      try {
        await updateDoc(doc(db, userDocPath), {
          spts: increment(1), // Credit 1 Star Point (spts) instead of stars
          updatedAt: serverTimestamp()
        });
        
        try {
          (window as any).Telegram?.WebApp?.HapticFeedback?.notificationOccurred('success');
          (window as any).Telegram?.WebApp?.showAlert('Congratulations! You earned 1 Star Point! ⭐ Earn 10 to Spin!');
        } catch {}
      } catch (error) {
        handleFirestoreError(error, OperationType.UPDATE, userDocPath);
      } finally {
  ...
  const handleSpin = async () => {
    ...
    try {
      if (resultType === 'stars') {
        await updateDoc(doc(db, userDocPath), {
          stars: increment(prize),
          spts: 0, // Spend the 10 spts
          updatedAt: serverTimestamp()
        });
      } else if (resultType === 'try_again') {
        // "Reset the 10 spts to give user another chance" -> Effectively set to 10 again if we deducted, 
        // or just don't deduct. I'll spend then set to 10 for consistency.
        await updateDoc(doc(db, userDocPath), {
          spts: 10, 
          updatedAt: serverTimestamp()
        });
      } else {
        // Good Luck - Spend 10 spts
        await updateDoc(doc(db, userDocPath), {
          spts: 0,
          updatedAt: serverTimestamp()
        });
      ...
      try {
        (window as any).Telegram?.WebApp?.HapticFeedback?.notificationOccurred(resultType === 'stars' ? 'success' : 'warning');
        (window as any).Telegram?.WebApp?.showAlert(resultMsg);
      } catch {
        alert(resultMsg);
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen pb-28 bg-[#061B1B] font-sans selection:bg-[#10B981]/30 overflow-x-hidden">
      ...
      <header className="px-6 pt-6 pb-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">
            {activeTab === 'home' ? `Hello, ${userData?.username || 'User'}!` : activeTab === 'tasks' ? 'Tasks' : activeTab === 'stars' ? 'Win Star' : activeTab === 'invite' ? 'Invite' : activeTab === 'wallet' ? 'Withdraw' : 'Profile'}
          </h1>
          <p className="text-sm text-[#A0AEC0] mt-0.5">
            {activeTab === 'home' ? "Let's earn some points(USD) today!" : activeTab === 'tasks' ? "Complete tasks to earn more" : activeTab === 'stars' ? "Earn Exclusive Stars" : activeTab === 'wallet' ? "Cash out your earnings" : "Refer friends to get paid"}
          </p>
        </div>
        ...
        {activeTab === 'home' ? (
          <>
            ...
            >
              <div className="relative z-10">
                ...
                <div className="mt-8 grid grid-cols-4 gap-2 border-t border-white/20 pt-6">
                  <div className="text-center">
                    <p className="text-[10px] uppercase font-bold opacity-60 tracking-wider">Balance</p>
                    <p className="text-sm font-bold mt-1 text-[#10B981]">{Math.floor(profile?.balance || 0)}</p>
                  </div>
                  <div className="text-center border-x border-white/10 px-2">
                    <p className="text-[10px] uppercase font-bold opacity-60 tracking-wider">Stars</p>
                    <p className="text-sm font-bold mt-1 text-yellow-500">{profile?.stars || 0} ⭐</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] uppercase font-bold opacity-60 tracking-wider">Friends</p>
                    <p className="text-sm font-bold mt-1">{profile?.total_invites || 0}</p>
                  </div>
                  <div className="text-center border-l border-white/10 pl-2">
                    <p className="text-[10px] uppercase font-bold opacity-60 tracking-wider">Ads</p>
            ...
            {/* Win Stars Sneak Peek */}
            <section className="bg-gradient-to-r from-yellow-500/10 to-transparent rounded-2xl p-4 flex items-center gap-4 cursor-pointer border border-yellow-500/10 shadow-[0_4px_20px_rgba(234,179,8,0.05)]" onClick={() => setActiveTab('stars')}>
              <div className="w-12 h-12 rounded-xl bg-yellow-500/20 flex items-center justify-center">
                <Star className="w-6 h-6 text-yellow-500 fill-current" />
              </div>
              <div className="flex-1">
                <h4 className="font-bold text-sm text-white">Win Star Section</h4>
                <p className="text-xs text-yellow-500/60 font-medium">Earn exclusive stars for rewards</p>
              </div>
              <ArrowUpRight className="w-5 h-5 text-yellow-500/40" />
            </section>
          </>
        ) : activeTab === 'tasks' ? (
          ...
          <div className="space-y-6">
            ...
            <motion.div 
              ...
              className="bg-gradient-to-br from-yellow-400/20 via-yellow-500/10 to-transparent rounded-[32px] p-8 border border-yellow-500/20 relative overflow-hidden text-center"
            >
              <div className="relative z-10">
                <div className="w-20 h-20 bg-yellow-500/20 rounded-full flex items-center justify-center mx-auto mb-4 shadow-[0_0_30px_rgba(234,179,8,0.2)]">
                  <Star className="w-10 h-10 text-yellow-500 fill-current" />
                </div>
                <h2 className="text-4xl font-black text-white">{profile?.stars || 0} Stars</h2>
                <p className="text-sm text-yellow-500/80 font-bold mt-1 uppercase tracking-widest">Premium Balance</p>
              </div>
              
              <div className="absolute -right-10 -top-10 w-32 h-32 bg-yellow-500/10 rounded-full blur-3xl" />
              <div className="absolute -left-10 -bottom-10 w-32 h-32 bg-yellow-500/10 rounded-full blur-3xl" />
            </motion.div>

            {/* Spin Progress Section */}
            <section className="stats-card rounded-[24px] p-6 border border-white/5 space-y-4">
              <div className="flex items-center justify-between">
                 <h3 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2">
                   <Zap className="w-4 h-4 text-yellow-500" />
                   Spin Progress
                 </h3>
                 <span className="text-xs font-black text-yellow-500">{profile?.spts || 0}/10 SPTS</span>
              </div>
              
              <div className="w-full h-3 bg-white/5 rounded-full overflow-hidden border border-white/5">
                <motion.div 
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.min(((profile?.spts || 0) / 10) * 100, 100)}%` }}
                  className="h-full bg-gradient-to-r from-yellow-600 to-yellow-400"
                />
              </div>
              <p className="text-[10px] text-[#A0AEC0] text-center font-medium">Earn 10 Star Points to unlock the SPIN WHEEL!</p>
            </section>
            ...
            <section className="space-y-4">
              ...
              <div className="stats-card rounded-[24px] p-6 border border-white/5 space-y-6">
                <div className="flex gap-4 items-start">
                  <div className="w-10 h-10 rounded-xl bg-yellow-500/10 flex items-center justify-center shrink-0">
                    <MonitorPlay className="w-5 h-5 text-yellow-500" />
                  </div>
                  <div className="flex-1">
                    <h5 className="font-bold text-sm text-white">Watch Star Ads</h5>
                    <p className="text-[11px] text-[#A0AEC0] leading-relaxed mt-1">
                      Watch a special video ad to earn 1 Star Point (SPTS).
                    </p>
                    <button 
                      onClick={handleWinStar}
                      disabled={isWatching}
                      className="mt-4 px-6 py-2.5 rounded-xl bg-yellow-500 text-black font-black text-xs flex items-center gap-2 shadow-lg shadow-yellow-500/20 disabled:opacity-50"
                    >
                      {isWatching ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4 fill-current" />}
                      WATCH STAR AD (+1 SPTS)
                    </button>
                  </div>
                </div>
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="flex-1">
                    <h5 className="font-bold text-sm text-white">Referral Milestones</h5>
                    <p className="text-[11px] text-[#A0AEC0] leading-relaxed mt-1">
                      Earn 5 Star Points for every 10 active friends you invite. (Coming Soon)
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white/5 rounded-[24px] p-5 border border-white/10">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h4 className="font-bold text-sm text-white">Star Benefits</h4>
              </div>
              <ul className="space-y-2">
                {[
                  "Unlock exclusive withdrawal methods",
                  "Lower withdrawal fees",
                  "Priority support access",
                  "Special badge in leaderboards"
                ].map((item, i) => (
                  <li key={i} className="flex items-center gap-2 text-[10px] text-[#A0AEC0]">
            ...
            <div className="bg-white/5 rounded-[32px] p-8 border border-white/10 relative overflow-hidden">
              <div className="relative z-10">
                <div className="flex items-center gap-6 mb-10">
                  ...
                  <div className="bg-black/30 p-4 rounded-2xl border border-white/5">
                    <p className="text-[10px] font-black opacity-40 uppercase tracking-widest text-[#A0AEC0]">Stars Earned</p>
                    <p className="text-xl font-black text-yellow-500 mt-1">{profile?.stars || 0} ⭐</p>
                  </div>
                  <div className="bg-black/30 p-4 rounded-2xl border border-white/5">
                  ...
                  <div className="bg-black/30 p-4 rounded-2xl border border-white/5">
                    <p className="text-[10px] font-black opacity-40 uppercase tracking-widest text-[#A0AEC0]">Goal Progress</p>
                    <p className="text-xl font-black text-[#10B981] mt-1">{profile?.adsSinceLastWithdrawal || 0}/25</p>
                  </div>
                </div>
      ...
      {/* Navigation Bar */}
      <nav className="fixed bottom-0 left-0 right-0 py-4 pb-8 px-2 nav-blur z-50">
        <div className="max-w-md mx-auto flex items-center justify-between">
          <NavItem icon={<Home />} label="Home" active={activeTab === 'home'} onClick={() => setActiveTab('home')} />
          <NavItem icon={<Zap />} label="Tasks" active={activeTab === 'tasks'} onClick={() => setActiveTab('tasks')} />
          <NavItem icon={<Star />} label="Stars" active={activeTab === 'stars'} onClick={() => setActiveTab('stars')} />
